            ...
            <tbody className="divide-y divide-slate-100">
              {b2bInvoices.map(inv => (
                <tr key={inv._id || inv.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-right font-medium text-rose-600">Γé╣{inv.amount.toLocaleString()}</td>
                    ...
                    {orderItems.map((item, index) => (
                      <div key={item.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group">
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100 flex justify-between items-center">
                          <p className="text-xs text-slate-500">Unit: Γé╣{item.price?.toLocaleString() || 0}</p>
                          <p className="text-sm font-bold text-slate-800">
                            Total: Γé╣{item.amount?.toLocaleString() || (item.price * item.quantity).toLocaleString()}
                          </p>
                        </div>
