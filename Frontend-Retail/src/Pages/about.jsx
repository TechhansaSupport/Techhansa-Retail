...
// --- NEW COMPONENT: FLOATING BACKGROUND SHAPES ---
...
export default function About() {
  ...
  return (
    <div className="bg-white min-h-screen text-[var(--text-dark)] font-['Inter',sans-serif] overflow-hidden relative">

      {/* 0. TOP SCROLLING BANNER WITH SMALL 3D ELEMENT */}
      <section ref={bannerRef} className="relative bg-[var(--text-dark)] overflow-hidden z-20">
      ...
      <section className="relative pt-32 pb-20 px-6 bg-gradient-to-b from-white to-[var(--soft-bg)] z-10">
        <FloatingShapes /> {/* Injected floating shapes */}
        <div className="container mx-auto text-center max-w-4xl relative z-10">
          <motion.div initial="hidden" animate="visible" variants={staggerContainer}>
            <motion.h1 variants={fadeUp} className="text-4xl md:text-5xl lg:text-5xl font-extrabold text-[#835e2e] mb-6 tracking-tight">
              Empowering Businesses with <br />
              <span className="text-[var(--premium-gold)]">Smarter IT Procurement</span>
            </motion.h1>
            <motion.p variants={fadeUp} className="text-lg md:text-xl text-slate-600 leading-relaxed mb-8">
        ...
        {/* Subdued original background rings */}
        <div className="absolute top-20 left-10 w-72 h-72 border-[1px] border-[var(--premium-gold)]/10 rounded-full blur-sm"></div>
        <div className="absolute bottom-10 right-10 w-[30rem] h-[30rem] border-[2px] border-[var(--premium-gold)]/5 rounded-full blur-md"></div>
      </section>

      {/* 2. WHO WE ARE, MISSION, VISION */}
      <section className="py-20 px-6 bg-white relative z-10">
        <div className="container mx-auto max-w-6xl">
          <motion.div className="grid md:grid-cols-3 gap-12" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={staggerContainer}>
            <motion.div variants={fadeUp} className="md:col-span-1 space-y-6">
              <h2 className="text-3xl font-bold text-[var(--text-dark)] border-l-4 border-[var(--premium-gold)] pl-4">Who We Are</h2>
              <p className="text-slate-600 leading-relaxed">
                At Techhansa Retail, we believe that business technology procurement should be simple, transparent, and value-driven. Traditional purchasing often involves multiple vendors, inconsistent pricing, and lengthy negotiations. Our platform transforms this experience.
              </p>
            </motion.div>
            <motion.div variants={fadeUp} className="md:col-span-2 grid sm:grid-cols-2 gap-8">
              <div className="bg-[var(--soft-bg)] p-8 rounded-2xl border border-[#2A85D1]/10 shadow-sm hover:shadow-md hover:border-[#2A85D1]/30 transition-all">
                <h3 className="text-xl font-bold text-[var(--tech-blue)] mb-3">Our Mission</h3>
                <p className="text-slate-600 text-sm leading-relaxed">
                  To revolutionize B2B IT hardware procurement by creating a transparent, technology-driven platform that connects buyers, suppliers, partners, and franchise businesses through innovation, efficiency, and trust.
                </p>
              </div>
              <div className="bg-[var(--soft-bg)] p-8 rounded-2xl border border-[#2A85D1]/10 shadow-sm hover:shadow-md hover:border-[#2A85D1]/30 transition-all">
                <h3 className="text-xl font-bold text-[var(--tech-blue)] mb-3">Our Vision</h3>
                <p className="text-slate-600 text-sm leading-relaxed">
                  To become India's most trusted and preferred B2B IT hardware procurement ecosystem by enabling seamless collaboration between organizations, suppliers, partners, and franchise entrepreneurs.
                </p>
              </div>
            </motion.div>
          </motion.div>
      ...
      {/* 3. WHAT WE DO (PRODUCTS) & SMART BIDDING */}
      <section className="py-20 px-6 bg-[var(--soft-bg)] relative z-10">
        <div className="container mx-auto max-w-6xl">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={staggerContainer}>
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-3 mb-20">
              {products.map((item, idx) => (
                <span key={idx} className="px-5 py-2 bg-white border border-[#2A85D1]/20 text-slate-700 rounded-full text-sm font-medium shadow-sm hover:border-[var(--tech-blue-hover)] hover:text-[var(--tech-blue-hover)] hover:bg-[var(--soft-bg)] transition-all cursor-default">
                  {item}
                </span>
              ))}
            </motion.div>
            {/* Smart Bidding Feature Card */}
            <motion.div variants={fadeUp} className="relative group max-w-4xl mx-auto">
              ...
              <div className="relative bg-white p-10 md:p-14 rounded-3xl border border-[#2A85D1]/20 shadow-[0_8px_30px_rgba(42,133,209,0.08)] text-center">
      ...
      {/* 4. OPPORTUNITIES (PARTNER & FRANCHISE) */}
      <section className="py-20 px-6 bg-white relative z-10">
        <div className="container mx-auto max-w-6xl">
          <motion.div className="grid md:grid-cols-2 gap-10" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={staggerContainer}>
            {/* Partner Portal */}
            <motion.div variants={fadeUp} className="bg-[var(--soft-bg)] p-10 rounded-3xl border border-transparent hover:border-[#2A85D1]/30 transition-colors">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={staggerContainer}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
              {whyChooseUs.map((reason, idx) => (
                <motion.div key={idx} variants={fadeUp} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 text-left hover:shadow-md transition-shadow">
            ...
            <div className="grid md:grid-cols-5 gap-6">
              {coreValues.map((value, idx) => (
                <motion.div key={idx} variants={fadeUp} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:border-[#DDA73C]/50 transition-colors">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} variants={staggerContainer}>
              ...
              {steps.map((step, index) => (
                <motion.div key={index} variants={fadeUp} className={`flex flex-col md:flex-row items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                  ...
                  <div className={`w-full md:w-1/2 pl-14 md:pl-0 ${index % 2 === 0 ? 'md:pr-12 text-left md:text-right' : 'md:pl-12 text-left'}`}>
                    <div className="bg-[var(--soft-bg)] p-6 rounded-2xl border border-transparent hover:border-[#2A85D1]/20 transition-all">
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {whoCanJoin.map((group, idx) => (
                <motion.div key={idx} variants={fadeUp} className="bg-[var(--soft-bg)] p-8 rounded-2xl border border-transparent hover:border-[#DDA73C]/40 text-center transition-all">
      ...
      {/* 7. COMMITMENT & CTA */}
      <section className="py-24 px-6 bg-gradient-to-b from-white to-[var(--soft-bg)] border-t border-slate-100 relative z-10">
        <div className="container mx-auto max-w-4xl text-center">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
            ...
            <motion.div variants={fadeUp} className="bg-[var(--tech-blue)] p-10 rounded-3xl shadow-xl shadow-[#2A85D1]/20 transform transition duration-300 hover:scale-[1.02]">
              ...
              <p className="text-[var(--soft-bg)] mb-8">
